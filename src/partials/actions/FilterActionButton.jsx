...
function FilterButton({ title, options = [], onSelect, ...props }) {
  ...
  return (
    <div className="relative inline-flex">
      ...
      >
        <div ref={dropdown}>
          <div className="text-xs font-semibold text-slate-400 uppercase pt-1.5 pb-2 px-4">Filters {`(${originalOptions.length})`}</div>
          {
            originalOptions.length > 6 && (
              <input value={optionsFilterText} onChange={(e) => onOptionsFilter(e.target.value)} className='text-xs text-slate-700 border border-slate-300 mx-4 my-2 p-2 rounded w-5/6' />
            )
          }
          <ul className="mb-4">
            {
